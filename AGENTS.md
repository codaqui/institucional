...
| **GitHub-as-Database** | Token OAuth do próprio membro (scope `public_repo`, criptografado em repouso — `GITHUB_TOKEN_ENCRYPTION_KEY` + `GITHUB_REPO_*` no `.env.example`). ⚠️ **Legado:** hoje usado apenas pelo force-sync manual do snapshot interno (`POST /events/internal/snapshot`, abre PR em nome do membro — canônico se colaborador; fork flow senão; será removido, ver `docs/plans/events-sync-improvements/` Camada 3). Overrides e organizers migraram para o PostgreSQL (`event_overrides`, `event_organizer_ownership`) |
...
│   └── sync-social-stats.mjs    # Gera static/social-stats/ (Discord/Meetup/GitHub)
...
- **Páginas estáticas por evento (SEO/OG):** o plugin local `src/plugins/event-pages.ts` gera `/eventos/detalhe/<source>/<sourceId>/<id>` para cada evento do `static/events/index.json` (componente `src/components/EventDetailRoute/`, helpers em `src/utils/event-path.ts`), com OG/JSON-LD corretos para crawlers. A URL legada com query redireciona para a rota estática quando ela existe, exceto no retorno do Stripe (`status=success&session_id=...`). Detalhes em `docs/modules/events/CODE_MANUAL.md` §2.8.
