...
# Data sync scripts (require env vars — see .env)
npm run sync:events  # Sync events from Discord + Meetup → static/events/
npm run sync:social  # Sync social stats → static/social-stats/index.json
npm run sync         # Run both syncs sequentially
```
...
| `communities.ts` | `communities[]` (with `socialProfiles[]` per community) | index.tsx, sobre/ong.tsx, sobre/insights.tsx |
