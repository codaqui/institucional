...
| Storage | ⚠ **Evitado** — comprovantes ficam no **Google Drive (manual)**. Sem S3/MinIO/disco local. |
...
| `StorageModule` | Validação de URLs de comprovante — apenas **valida o formato** das URLs informadas pelo time (que sobe os arquivos manualmente no **Google Drive**). Nenhum upload acontece pelo backend. | `POST /storage/validate-receipt-url` |
